...
def runRS(R):
    ...
    csvfile = open("rs2l_edrun2.txt", "a")
    for tick in range(len(evals)):
        csvfile.write(str(evals[tick]) + ',')
        csvfile.write(str(dis[tick]) + '\n')
    csvfile.close()

    csvfile1 = open("rs2l_betterpointsrun2.txt", "a")
    for tic in range(len(allbetterSd)):
        csvfile1.write(str(allbetterSd[tic]) + '\n')
    csvfile1.close()
